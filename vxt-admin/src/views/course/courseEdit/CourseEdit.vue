                ...
                <div class="head">
                    <div class="head-wrapper">
                        ...
                        <div class="head-extra">
                            <el-button type="success" @click="submitEditCourse">保存</el-button>
                        </div>
                    </div>
                </div>
                <div class="body">
                    <div class="form-box">
                        <el-form :model="editForm" :rules="rules" label-position="right" label-width="80px">
                            <el-form-item label="课程ID" prop="id">
                                <el-input v-model="editForm.id" placeholder="请填写课程ID" disabled></el-input>
                            </el-form-item>
                            <el-form-item label="课程标题" prop="title">
                                <el-input v-model="editForm.title" placeholder="请填写课程标题"></el-input>
                            </el-form-item>
                            ...
                            <el-form-item label="所属讲师" prop="teacher_id">
                                <el-select v-model="editForm.teacher_id" placeholder="请选择所属讲师" class="from-item" style="width:100%">
                                    <el-option v-for="item in ownTeachers" :key="item.id" :label="item.name"
                                               :value="item.id"></el-option>
                                </el-select>
                            </el-form-item>
                            <el-form-item label="所属分类" prop="category_id">
                                <el-select v-model="editForm.category_id" placeholder="请选择所属分类" class="from-item" style="width:100%">
                            ...
                            <el-form-item label="课程封面" prop="fm_url">
                                ...
                                <div class="upload-img" v-show="editForm.fm_url">
                                    <div class="img">
                                        <el-image :src="baseURL + editForm.fm_url" :fit="fit" alt="暂无图片"></el-image>
                                    </div>
                            ...
                            <el-form-item label="课程简介" prop="intro">
                                <TEditor v-if="editForm.intro" v-model="editForm.intro"/>
                            </el-form-item>
                        </el-form>
                    </div>
                </div>
    ...
    methods: {
        ...
        getAllData() {
            Promise.all([
                ...
                this.editForm = response[2].data.data
            }).catch(error => {
                console.log(error)
        ...
        uploadSuccess(response) {
            ...
            this.editForm.fm_url = response.data.file_path
        },
        // 图片上传失败
        uploadError() {
        ...
        deleteUploadImg() {
            ajax(
                ...
                {
                    file_path: this.editForm.fm_url
                }
            ).then(response => {
                console.log(response)
                if (response.data.successful) {
                    this.$message.success(response.data.msg)
                    this.editForm.fm_url = ''
        ...
        submitEditCourse() {
            ajax(
                'manager/course/course_update',
                {
                    id: this.editForm.id,
                    title: this.editForm.title,
                    fm_url: this.editForm.fm_url,
                    is_hot: this.editForm.is_hot,
